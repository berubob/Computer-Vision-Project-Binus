...
function getMaintenanceDesc(status: string) {
  switch (status) {
    case 'CRITICAL': return 'Immediate reconstruction required within 7 days.';
    case 'SERIOUS':  return 'Schedule patching and repair within 30 days.';
    case 'WARNING':  return 'Preventive maintenance recommended within 90 days.';
    default:         return 'No action needed. Routine inspection in 6 months.';
  }
}
...
function getMaintenancePriority(status: string) {
  switch (status) {
    ...
    case 'SERIOUS':  return 'High Priority';
    ...
    <div ref={reportRef} className="min-h-screen bg-[#0d111a] text-white p-6 font-sans">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            ...
            <span className="px-3 py-1 border border-gray-700 bg-gray-800/50 text-gray-400 rounded-full text-xs font-semibold">
              {result.potholes_detected} Potholes Detected
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6">
        ...
        <div className="lg:col-span-3 bg-[#111723] border border-gray-800 rounded-2xl p-6 flex flex-col justify-between">
          <h3 className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-4">Road Health Index</h3>
          ...
          <div className="mt-4 border-t border-gray-800/60 pt-4">
            <p className="text-gray-400 text-xs leading-relaxed">
              Road health is at <span className="text-amber-500 font-semibold">{result.rhi}%</span> with a max pothole score of <span className="text-amber-500 font-semibold">{result.mps}/100</span>.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 bg-[#111723] border border-gray-800 rounded-2xl p-6 flex flex-col gap-4">
          <h3 className="text-gray-400 text-xs font-bold uppercase tracking-wider">Detection Result</h3>

          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-gray-500 text-xs">Potholes Detected</span>
              <span className="text-white text-xs font-semibold">{result.potholes_detected ?? 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-500 text-xs">Road Health Index</span>
          ...
          <div className="flex flex-col gap-2">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Maintenance Priority</p>
        ...
        <div className="lg:col-span-6 bg-[#111723] border border-gray-800 rounded-2xl p-6">
          ...
          {imageUrl ? (
            ...
            <div className="w-full h-44 rounded-xl bg-[#0d111a] border border-gray-800 flex items-center justify-center">
              <span className="text-gray-600 text-xs">No visual output</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-center">
        <div className="w-full max-w-2xl bg-[#111723] border border-gray-800 rounded-2xl p-6">
          ...
          <p className="text-gray-500 text-xs mb-6">
            Based on detection results: {result.potholes_detected ?? 0} potholes found
          </p>
